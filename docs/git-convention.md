...
AngularJS Commit Convention을 따른다. 제목과 부연 설명은 반드시 **별개의 줄**로 작성하고, 그 사이에 **빈 줄을 하나 넣는다.** 한 줄에 ` - ` 로 이어 붙이면 git 이 전체를 제목으로 합쳐 GitHub UI 에서도 본문이 제목에 박힌 채로 나오므로 금지한다. 빈 줄을 빠뜨려도 결과는 같다 — git 은 첫 빈 줄까지를 제목으로 보므로, 빈 줄이 없으면 부연 설명까지 제목에 딸려 들어간다.
...
**잘못된 예** — 빈 줄이 없어 부연까지 제목이 됨
```
feat: DEX 스왑 시뮬레이션 기능 추가
- 슬리피지 검증 및 가스비 차감 로직 구현
```

두 줄짜리 메시지는 heredoc 으로 실제 줄바꿈과 빈 줄을 넣는다. `git commit -m "제목 - 부연"` 처럼 한 문자열로 넘기면 위 잘못된 예가 된다.
...
커밋 후 `git log --format='%h %s' -1` 로 제목만 나오는지 확인한다. 부연 설명이 함께 보이면 빈 줄이 빠진 것이다.
