...
| Step | Reader | Processor | Writer |
|------|--------|-----------|--------|
| reportStep | ACTIVE 라운드의 위반 기록 + 주문 이력 | 위반분 우선 매칭, loss 계산, 시나리오 생성 | REGRET_REPORT + SCENARIO + VIOLATION_TRADE 저장 |
...
2. `REGRET_REPORT` + `RULE_SCENARIO` + `VIOLATION_TRADE`를 upsert한다

### 갱신 정책

- ACTIVE 라운드: 매일 갱신 (새로운 위반이 추가될 수 있으므로)
- ENDED 라운드: 종료 시 1회 확정 생성 후 변하지 않음
- 리포트가 없으면 (라운드 시작 당일) API 조회 시점에 생성하고 저장한다
