...
| 3 | `TickerEventConflator.submit()` | RabbitMQ `ticker.exchange` (50ms conflation) |
...
| 항목 | 값 |
|------|-----|
| 발행 단위 | 1 메시지 = 1 거래소의 50ms 윈도우 batch |
| Publisher Confirms | `correlated` 모드 (nack 시 로그 경고) |
| 에러 처리 | 직렬화/발행 실패 시 로그 경고 (시세 수집을 중단하지 않음) |

**Conflation + Batching** — `TickerEventConflator` 가 `(exchange, base, quote)` 별 slot 1칸을 유지하다가 50ms 마다 `@Scheduled` 로 flush 한다. 같은 키로 50ms 안에 N건 들어오면 마지막 1건만 batch 에 들어가고, batch 는 거래소별로 그룹화되어 거래소당 1 메시지로 `TickerEventPublisher.publishBatch()` 가 발행한다. marketdata 채널만 대상이며, 매칭 엔진 tick(`engine.inbox`)은 그대로 매 건 발행된다. 셧다운 시 `@PreDestroy` 가 마지막 1회 flush 한다.
