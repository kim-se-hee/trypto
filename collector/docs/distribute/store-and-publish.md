...
| 3 | `TickerEventPublisher.publishBatch()` | RabbitMQ `ticker.exchange` |
...
| `candle_1M_kst` | `0 15 * * *` (매일) | 매월 1일 00시 KST |

월봉 경계는 매월 말일 15시 UTC 라 cron 한 줄로 표현되지 않는다. 대신 매일 KST 자정에 월중 누계를 재계산한다. `last()` 는 마지막 윈도우 하나만 남기므로 평소에는 진행 중인 달의 누계가 갱신되고, 말일 실행에서는 진행 중 윈도우 폭이 0 이라 완성된 한 달 윈도우가 확정 기록된다. 조회 구간 62일은 달 길이와 무관하게 직전 달 경계까지 항상 덮기 위한 값이다.
...
> 서버 재시작 시 다운타임 동안의 캔들 공백을 메우고 진행 중 봉을 복구하는 방식은 [candle-backfill.md](candle-backfill.md) 참조.
...
| 항목 | 값 |
|------|-----|
| 발행 단위 | tick 1건이 들어오는 즉시 크기 1 batch 로 1 메시지 발행 |
| Publisher Confirms | `correlated` 모드 (nack 시 로그 경고) |
| 에러 처리 | 직렬화/발행 실패 시 로그 경고 (시세 수집을 중단하지 않음) |

과거에는 50ms 윈도우 conflation(심볼별 최신 1건 유지 후 거래소별 묶음 발행)을 적용했으나, 팬아웃 부하가 문제되지 않는 규모에서 지연만 더한다고 판단하여 즉시 발행으로 되돌렸다. 페이로드 형식은 batch 배열 그대로 유지한다.
