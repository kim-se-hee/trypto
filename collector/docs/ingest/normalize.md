...
1. **마켓 목록 조회** — REST로 상장 코인 목록을 받아 `MarketInfoCache`(인메모리)와 Redis 양쪽에 적재한다.
   ...
   ```
2. **초기 시세 스냅샷** — REST로 현재가를 받아 Redis에 저장한다. WebSocket 첫 메시지가 도착하기 전 캐시 공백을 메우는 용도다.
   - 업비트·빗썸은 마켓 코드를 쿼리스트링에 싣는 방식이라 상장 종목이 늘면 URI 길이 한도(4KB)에 걸린다. 100개씩 나눠 요청하고 결과를 합친다.
   - 한 묶음이라도 실패하면 스냅샷 전체를 실패로 처리한다. 일부 종목만 갱신된 채로 남아 조용히 틀린 시세를 노출하는 것을 막기 위함이다.
