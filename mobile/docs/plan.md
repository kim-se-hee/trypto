...
본 문서는 설계안 3건(pragmatic / performance / maintainable)과 심사 3건(ticker-perf / auth-realtime / buildability)을 종합한 **아키텍처 결정문**이다. 기준 사양은 `mobile/docs/web-spec.md` 이다.

### 문서의 서열

**현행 동작의 기준은 `frontend/` 의 웹 코드와 `api/` 의 서버 계약이다.** 본 문서와 `web-spec.md` 는 모두 이식 착수 시점(2026-07-15)의 기록이며, 코드와 어긋나면 언제나 코드가 이긴다.

| 문서 | 성격 | 읽는 법 |
|---|---|---|
| `frontend/` · `api/` 코드 | **단일 진실 공급원** | 현행 동작이 궁금하면 여기부터 본다 |
| `web-spec.md` | 이식 시점의 웹·서버 사양 스냅샷 | 화면 규칙·계약의 사실관계를 확인할 때 본다 |
| 본 문서(`plan.md`) | 앱 **아키텍처 결정 근거** | "왜 그 구조를 골랐는가" 를 확인할 때 본다 |

본 문서가 사양서보다 앞서는 범위는 **아키텍처 결정**(상태 관리·라우팅·실시간 구조·테스트 범위 등)뿐이다. 웹 화면의 문구·포맷·계약 같은 사실관계는 사양서가, 사양서와 코드가 어긋나면 코드가 앞선다. 본 문서에 적힌 결정의 **근거**는 그 자체로 유효하되, 인용된 웹 동작이 그 뒤 바뀌었을 수 있다.
...
| OAuth | 이식 시점 결정은 방안 B(커스텀 스킴 콜백 `trypto://auth/{provider}/callback`)였으나, **현행 구현은 두 제공자 모두 공식 SDK 로 앱에서 직접 토큰을 받는다**(§4.3). 로그인 바디에 `clientType` 을 실어 서버가 플랫폼별 자격증명을 고르게 한다(`android`/`ios`/`web`) |
...
- **단, 마켓 위로 올라온 화면이 있으면 멈추지 않는다.** 코인 상세와 가로 풀스크린 차트는 마켓을 덮으면서도 같은 스토어에서 티커를 받는다. 그 화면들이 `hold()` 로 잡고 `release()` 로 놓는 **점유 카운터**를 두고, 카운터가 0 보다 크면 `setActive(false)` 를 무시한다.
- `release()` 는 **`State.dispose` 에서 호출되므로 그 안에서 `ref` 를 읽으면 안 된다.** 그 시점에는 위젯이 이미 해제되어 riverpod 이 예외를 던지고, 그러면 해제가 실행되지 않아 스토어가 영원히 깨어 있는다. 잡을 때 받은 스토어를 **필드로 들고 있다가** 그것으로 놓는다.
...
**표시 시점에는 서버 기준(KST) 벽시계로 되돌린다.** 컨버터는 시각을 기기 로컬로 옮기지만, 화면에 찍는 날짜·시각은 서버가 남긴 값과 같아야 한다.

| 헬퍼 | 쓰는 자리 |
|---|---|
| `kstWallClock(at)` — KST 벽시계 그대로 | 시·분까지 보여주는 자리(체결 시각, 위반 상세). 날짜로 자르면 `HH:mm` 이 늘 `00:00` 이 된다 |
| `kstDate(at)` — KST 기준 날짜로 절삭 | 날짜만 쓰는 자리(가입일, 라운드 시작일, 스냅샷 날짜). 기기 시간대로 찍으면 UTC−8 기기에서 하루 어긋나고, 서버가 `LocalDate` 로 내린 값과도 맞지 않는다 |
...
| 항목 | 내용 |
|---|---|
| 위젯 | 로고(앱 아이콘 + 소문자 "trypto") · 서브카피("기록으로 배우는 코인 모의투자") · 카카오 버튼(`#FEE500`/글자 `#191600`/높이 48, 라벨 "카카오로 로그인") · 구글 버튼(흰 배경 + 테두리 + 4색 SVG, 라벨 "구글로 로그인") · 오류 배너 — 문구는 웹 현행(§7.1.1)을 따른다 |
| API | `POST /api/auth/{provider}/login`. **현행은 SDK 토큰 채널**(`{accessToken, clientType}`) — 카카오는 액세스 토큰, 구글은 ID 토큰을 같은 필드로 보낸다. 인가 코드 흐름(`{code, codeVerifier, clientType}`)은 웹 경로로 남아 있다(§4.3) |
| 상태 | `AuthController`(`AuthStatus` 상태 기계). `exchanging` 동안 전체 차단 오버레이(로고 + 24×24 스피너 + "{카카오\|구글}로 로그인 중…") |
| 모바일 UX | **소셜 콜백 화면을 만들지 않는다.** 팝업·BroadcastChannel·bfcache 처리는 통째로 삭제한다 |
| 검증 실패 | §2.2.4 의 6종 메시지를 그대로 오류 배너에 표시한다 |
| **사용자 취소** | **실패로 다루지 않는다.** 제공자 SDK 가 알려 오는 취소(구글 `canceled`, 카카오 `access_denied` 또는 네이티브 `CANCELED`)는 별도 예외로 갈라 **오류 배너 없이 버튼만 되돌린다.** 웹도 인가 팝업이 그냥 닫히면 오류를 세우지 않는다(§2.2.5). 상속 관계에 가려지지 않도록 취소 분기를 일반 실패 분기보다 **앞**에 둔다 |
...
| 1단계 · 자금 | **거래소별 시드머니 카드 3장** — 각 카드는 그 거래소의 기축통화로 입력받는다(업비트·빗썸 `₩`, 바이낸스 `USDT`). 대형 모노 표시 + KRW 카드에만 `formatKRW` 축약 보조 표시. 프리셋은 KRW 100만/500만/1,000만/5,000만, USDT 1,000/5,000/10,000/30,000. 이어서 긴급 자금 상한 카드(원화 기준, 프리셋 10만/50만/100만). 커스텀 숫자 키패드(0~9, ⌫, 00) |
...
| API | `POST /api/rounds` `{seeds: [{1, upbitSeed}, {2, bithumbSeed}, {3, binanceSeed}], emergencyFundingLimit, rules: [...]}` — **`userId` 를 보내지 않는다.** 금액은 거래소 기축통화 단위이며, **시드 0 인 거래소도 반드시 함께 싣는다**(서버는 이 목록에 실린 거래소에만 지갑을 만든다 — 빠뜨리면 송금받을 지갑이 없다) |
| 선제 검증 (R8) | **긴급 자금 상한 1,000,000원을 입력 단계에서 강제**(초과 시 "상한은 100만원입니다") · **동일 `ruleType` 중복 전송 차단**(서버가 400 이 아니라 500 을 낸다) · 시드 0 초과 시 거래소별 범위(**KRW 100만~5,000만, USDT 1,000~30,000**) |
| 제출 조건 | **거래소 중 하나 이상 시드 > 0** && 모든 거래소의 시드가 범위를 통과 && `emergencyLimit > 0` && 활성 규칙 ≥ 1 |
...
| 리스트 | `ListView.builder(itemExtent: 68)`. 컬럼: 코인명(심볼 13/w600 + 한글명 11) · 현재가 · 전일대비 배지 · 거래대금. `currentPrice <= 0` 이면 현재가·전일대비를 함께 `-`, `volume <= 0` 이면 거래대금을 `-`. **거래대금은 축약하지 않는다**(`formatVolume`, §8.5.8) — 웹과 같은 표기이므로 조 단위 열세 자리가 들어갈 고정 폭을 확보한다 |
| 빈 상태 | 검색·필터 결과가 없으면 남은 높이 **한가운데**에 "검색 결과가 없습니다." 를 둔다. 고정 여백으로 밀어내면 목록 영역이 낮은 기기에서 문구가 화면 밖으로 나간다. 비어 있어도 당겨서 새로고침은 살아 있어야 한다 |
| 컨트롤 줄 | 필터 칩과 정렬은 **목록의 로딩·오류 분기 바깥**에 둔다. 웹의 필터 칩도 목록 상자가 아니라 상단 컨트롤 줄에 있어 로딩 중에도 남는다 |
...
| 긴급 자금 | 상단 **접힌 배너**(남은 횟수만). 탭 → 바텀시트. **투입할 거래소를 고르고**(기축통화 라벨 함께 표시, 바꾸면 금액 초기화) 그 거래소의 기축통화로 입력한다. 프리셋 25/50/100% 와 상한 비교는 **원화 거래소에서만** — 상한이 원화 기준이라 USDT 는 양수만 확인하고 서버가 투입 시점 시세로 환산해 검증한다(§4.5). `idempotencyKey` UUID v4. **실패 시 스낵바**(웹은 조용히 실패한다) |
...
- **과거 구간 페이징**(§4.3.11): 뷰포트가 바뀔 때마다 왼쪽 끝이 `kPrefetchThreshold`(8) 개 안으로 들어왔는지 보고, 들어왔으면 가장 오래된 봉을 커서로 이전 구간을 당겨 와 앞에 붙인다. 앞에 끼운 만큼 **뷰포트 앵커와 제스처 기준점을 같이 민다** — 안 그러면 손끝과 화면이 어긋난다. 조회가 비면 끝에 다다른 것으로 보고 더 부르지 않고, 실패하면 다음 스와이프에서 다시 시도한다. 이 조회는 Riverpod 그래프를 타지 않는다(같은 키로 커서만 바꿔 여러 번 부르므로 캐시할 것이 없다).
- y축 가격 라벨과 툴팁의 시·고·저·종은 웹 `formatAxisLabel` 과 같다 — `getCurrencySymbol + formatPrice`, 즉 **억/만 축약을 하지 않는다**(§4.3.3). 축약하면 2천원 단위가 사라지고 USDT 저가 코인에서는 눈금 네 개가 같은 값으로 뭉개진다. 대신 `₩163,502,000` 열세 글자가 들어갈 폭을 오른쪽 패딩에 확보한다 — 웹의 124px 은 960px 캔버스 기준이라 모바일 폭에 그대로 쓸 수 없다.
...
`CandleRequest` 는 `(exchangeCode, symbol, interval)` 의 값 동등 레코드다. 이것이 웹의 `requestKey` 를 대신한다(§4.3.7) — 코인·거래소·간격이 바뀌면 provider 키가 바뀌어 이전 캔들이 화면에 남지 않고, `LiveCandleFolder` 도 같은 키를 들고 있다가 키가 어긋나면 **빈 배열에서 다시 시작한다.** 조회 개수(`limit`)·최초 표시 개수와 **봉 경계 절삭 오프셋(`tzOffsetMinutes`)** 은 이 세 값에서 파생하므로 동등 비교에 넣지 않는다.
...
```dart
// TickerStore (§4.2.4) 에 추가하는 것은 관찰자 목록과 그 순회뿐이다.
final List<TickObserver> _observers = [];            // 캔들 폴더들이 살아 있는 동안 등록된다

void addRawObserver(TickObserver o) {                // 같은 관찰자를 두 번 넣지 않는다
  if (_observers.contains(o)) return;                // 넣으면 한 체결이 두 번 접힌다
  _observers.add(o);
}
void removeRawObserver(TickObserver o) => _observers.remove(o);   // 자기 것만 뗀다
...
void ingest(List<Ticker> ticks) {
  ...
  for (final t in ticks) {
    ...
    for (var i = 0; i < _observers.length; i++) {
      _observers[i].onTick(t);   // ★ 매 틱. 캔들 접기(O(1), 할당 없음). 그리기는 하지 않는다
    }
...
- **관찰자는 여럿이 동시에 산다.** 슬롯 하나로는 부족하다 — 코인 상세 차트가 살아 있는 채로 가로 풀스크린이 그 위에 올라오고, 풀스크린에서 간격을 바꾸면 폴더가 하나 더 생긴다. 슬롯이 하나면 나중 것이 앞 것을 덮고, 풀스크린을 닫는 순간 아무도 남지 않아 상세 차트의 실시간 갱신이 멈춘다. 폴더는 자기 심볼·자기 간격으로만 접으므로 여럿이 살아도 서로 오염되지 않는다.
- 등록·해제는 폴더를 만든 provider 가 맡는다(`addRawObserver` / `onDispose` 에서 `removeRawObserver`). **해제는 자기 것만 뗀다** — 옛 관찰자의 해제가 새 관찰자를 지우면 안 된다.
- flush 안에서 `onFrame()` 을 돌 때도 인덱스로 순회한다. 알림을 받은 관찰자가 그 자리에서 자기를 뗄 수 있는데, 인덱스 순회는 순회 중 변경이 예외로 번지지 않는다.
- 코인 상세는 마켓 셸 브랜치 **위로 push** 되지만, 마켓 페이지는 자기가 가려지면 `setActive(false)` 를 부른다. 상세·풀스크린 차트도 같은 스토어에서 티커를 받으므로 그때 멈추면 안 된다 → **`hold()`/`release()` 로 잡아 둔다**(§4.2.3). **차트가 별도로 토픽을 구독하지 않는다.**
...
class LiveCandleFolder {
  ...
  void fold(Ticker t) {                              // ← TickerStore.ingest 가 매 틱 호출
    ...
    // §4.3.5.1. 절삭 기준은 단말 로컬이 아니라 **거래소 시간대**다(빗썸만 KST +540).
    final b = normalizeCandleTime(t.timestamp, key!.interval, key!.tzOffsetMinutes);
...
- **응답으로 서버 배열을 통째로 갈아 끼우지 않는다.** 재조회는 최신 창만 돌려주므로, 통째로 바꾸면 과거 페이징(§4.3.11)으로 앞에 쌓아 둔 구간이 사라진다. 겹치는 구간만 새 값으로 덮고 앞쪽은 유지한다. 같은 응답으로 다시 빌드될 때는 배열을 새로 만들지 않는다 — 새로 만들면 페인터의 `identical` 게이트가 깨져 틱마다 다시 칠한다.
...
- **지정가 칸은 현재가로 미리 채운다**(§4.4.3). 사용자가 손대기 전까지는 시세를 따라가고, 손댄 뒤로는 덮어쓰지 않는다. 매수↔매도 탭을 바꾸면 세 입력이 비므로 가격도 다시 채운다. **프리필된 값은 "입력했다" 로 세지 않는다** — 세면 시트를 열자마자 "주문 수량을 입력해 주세요." 가 붉게 뜬다.
- 수량↔총액 연동은 **지정가에서만**(`Decimal` 연산). 비율 버튼 10/25/50/100%. 가격 스텝 ±1,000.
- **수량은 반올림하지 않고 항상 내린다.** 서버 잔고는 소수 8자리 내림으로 관리되므로 반올림하면 `수량 × 가격` 이 총액을 넘어 실잔고보다 큰 값이 제출된다.
  - 총액 → 수량 환산: 표시 자릿수(6)로 **내림**.
  - 매도 비율 버튼: 표시 자릿수가 아니라 **잔고 자릿수(8)로 내림** — 전량 매도가 실잔고와 정확히 일치해야 한다.
  - 매수 100%: `총액 + 수수료 <= 잔고` 를 만족하는 최대 총액으로 가둔다. 서버는 수수료를 **기축통화 자릿수로 내림 절삭**(KRW 정수 원, USDT 8자리)하므로, `잔고 / (1 + 요율)` 내림에서 한 눈금씩 올려 절삭분을 되찾는다. 잔고 검사와 예상 수수료도 같은 식(`총액 + 절삭 수수료`)을 쓴다 — 반올림하면 서버보다 크게 잡혀 1원 차이로 매수가 막힌다.
  - 주문 가능 금액 표시도 **내린 뒤 포맷한다.** 순서를 바꾸면 포매터가 반올림해 실잔고보다 커 보이고, 그 값을 그대로 입력한 사용자가 거절당한다.
- 가격·체결가 표기는 **소수 8자리 전용 포매터**를 쓴다(§8.5.7 의 공용 `formatPrice` 는 KRW 에서 3자리라 1원 미만 코인이 뭉개진다). 웹도 `OrderPanel.tsx` 안에 같은 예외를 두고 있다.
...
**거래내역 탭**: 필터(체결/미체결), `GET /api/orders?...&size=20` 커서 무한 스크롤(`cursorOrderId`). **응답에 `status` 가 없으므로 요청 필터값을 표시에 쓴다.** `PENDING` 항목에 취소 버튼 → `POST /api/orders/{id}/cancel {walletId}`. 표기는 가격 8자리 · 수량 6자리 · 금액은 원화에서 **축약 없이 천 단위 구분만**(§4.4.9).

### 5.5 포트폴리오 (`/portfolio`)

| 항목 | 내용 |
|---|---|
| 구조 | `CustomScrollView` — ① 거래소 세그먼트 ② 요약 카드 ③ 자산 구성(도넛) ④ 정렬 칩 ⑤ 보유 종목 **카드 리스트** |
| 거래소 탭 | **이 라운드가 실제로 만든 지갑만** 세운다(`activeRound.wallets`, §5.0). 자금을 배정하지 않은 거래소는 탭도 없다. 쿼리가 없으면 지갑 목록의 첫 거래소가 기본값이다 — 업비트 지갑이 없는 라운드에서 빈 화면으로 시작하지 않는다 |
| API | `GET /api/wallets/{walletId}/portfolio` 1회. **폴링·WS 없음.** 셸이 이 화면을 살려 두므로 provider 캐시가 그대로 남는다 → **탭 재활성·거래소 전환·포그라운드 복귀에서 무효화**하고, `RefreshIndicator` 를 마지막 수동 경로로 둔다. 무효화하지 않으면 매수 체결 이전 스냅샷이 계속 보인다 |
...
| 자산 상세 | 카드 탭 → 바텀시트. 총 수량 · 환산액 · **[출금] 버튼(코인만)** · 잔고 상세 · 해당 코인 송금 내역. 다른 거래소 어디에도 상장되지 않은 코인은 출금 버튼을 **비활성**하고 사유를 적는다 |
| 송금 | **전체 화면 3단계 마법사**. ①도착 거래소(후보 1개면 자동 선택) ②수량(실시간 검증, `[25%][50%][최대]`) ③확인(출발→도착, 수량, **출금 후 남는 가용 잔고**) |
| 미상장 도착지 | 도착 거래소가 그 코인을 취급하지 않으면 서버가 `COIN_NOT_LISTED_ON_EXCHANGE` 로 막는다. 후보 목록에서 **지우지 않고 비활성화**하고 `{심볼} 미상장` 을 적어 이유를 남긴다(§5.2.6). 판정은 라운드 지갑이 걸친 거래소별 상장 코인 집합으로 하며, 목록이 아직 없는 거래소는 '미상장' 으로 본다. 거절 응답은 거래소명·심볼을 넣은 문구로 갈라 보여준다 |
...
| 랭커 포트폴리오 | 행 탭 → **바텀시트**(ExpansionTile 은 스크롤 위치를 흔든다). 유저별 1회 캐싱. 비중은 `asRatio(v) = v > 1 ? v/100 : v` 정규화 후 `(ratio×100).toFixed(1)%`. **종목별 수익률은 노출하지 않는다** — 웹도 심볼과 비중만 공개한다(§6.2.7) |
| 더보기 실패 | 무한 스크롤은 **실패한 뒤 자동으로 다시 쏘지 않는다.** 웹은 사용자가 버튼을 눌러야만 재시도하며, 이 차단이 없으면 목록 끝에 머무는 스크롤마다 같은 요청이 되풀이된다. 인라인 오류를 띄우고 재시도 수단을 사용자에게 넘긴다 |
...
| 항목 | 내용 |
|---|---|
| API | `GET /api/rounds/{roundId}/regret` + `/regret/chart` 를 병렬로. **복기는 라운드 단위**라 거래소를 가려 조회하지 않는다(§1.6.8) — 서버가 라운드의 모든 거래소를 합쳐 원화로 내린다 |
| 통화 | 요약·규칙별 손실·자산 곡선은 전부 **원화**다. 거래소 기축통화는 위반 거래 한 건 한 건에만 남으므로, 그 자리에서만 `exchangeName`·`currency` 를 함께 표기해 혼동을 막는다 |
| 히어로 | "위반 손실" 대형 숫자 + 한 줄 해설. **위반 손실은 양수가 손해**이므로 부호에 따라 색과 문구가 갈린다(§6.4.4). 그 아래 3-stat 타일(실제 / 규칙 준수 시 / 위반 건수). 비율은 **`toStringAsFixed(2)`** 로 다듬는다 |
...
| 시뮬레이션 | 켜 둔 규칙 몫의 위반 손실을 그날의 실제 자산에 직접 누적한다(§6.3.4). 미실현분은 금액으로 더하고 실현분은 **실현일 자산 대비 배수**로 환산해 곱한다(이중 차감 방지). 긴급 충전은 배수를 1 쪽으로 되돌린다. `sim[i] = ((actual[i] + occurred - realized) × multiplier).round()`. **동일 누적식·동일 반올림으로 이식해야 웹과 값이 일치한다.** 규칙 조합이 바뀌면 배수를 처음부터 다시 굴린다 |
| BTC 홀드 | 수익률은 `btcHoldValues` 의 `(last/first - 1) × 100`(스냅샷 2개 미만이거나 첫 값이 0 이면 표기하지 않는다). **규칙이 하나도 없어도 이 칩은 남긴다** — BTC 홀드 곡선은 리포트가 아니라 차트 데이터로 그려지므로 배치 전에도 선이 있는데, 칩이 사라지면 끌 수단도 값을 읽을 수단도 없다. 규칙이 비면 제목만 "만약 규칙을 지켰다면" → "벤치마크" 로 바꾼다 |
| 규칙 토글 | 세로 체크박스 → **가로 스크롤 `FilterChip` 행**(규칙 색 + 라벨 + **규칙별 손실 금액** + 위반 수 배지). 손실 0 과 위반 0 도 감추지 않는다 — "지켰다" 는 것도 정보다. 축약은 `formatCurrencyShort`(§8.5.12), 임계값 부호는 단위를 가리지 않고 양수면 `+`(`+3회`). 롱프레스 → 임계값·위반 상세 바텀시트 |
| 시각 표기 | 위반 목록의 날짜·체결 시각은 **서버 기준(KST) 벽시계**로 찍는다. 차트 마커가 KST 날짜로 버킷을 만들므로, 기기 시간대로 찍으면 같은 위반이 마커와 하루 어긋난다 |
| 위반 목록 | 필터 세그먼트(전체/손실/수익 + 건수). **위반 손실은 양수가 손해**이므로 손실은 `> 0`, **0 이하가 수익**이다. **2행 레이아웃**(1행: 코인·날짜·거래소·위반 손실 / 2행: 어긴 원칙과 원칙별 금액 가로 스크롤). 정렬하지 않는다(서버 순서 그대로). **감정 배지는 구현하지 않는다**(서버에 필드가 없다) |
...
| 프로필 | 닉네임 행 탭 → **하단 시트**(입력 + 저장). **2~20자를 입력 단계에서 검증**하고 실패는 시트 안에서 알린다(시트 뒤로 던진 스낵바는 가려서 보이지 않는다). 가입일은 `GET /api/users/me` 의 `createdAt` 을 **KST 날짜로 잘라** `2026년 7월 15일` 로 |
| 실패해도 버튼을 되살린다 | 닉네임 저장·피드백 전송처럼 버튼이 "저장 중…" 으로 잠기는 자리는 **어떤 예외가 나도** 잠금을 푼다(`finally`). 특정 예외형만 잡으면 그 밖의 예외에서 화면이 영영 굳는다. 이미 닫힌 시트는 되돌리지 않는다 — 닫히는 동안 라벨이 깜빡인다 |
| 예외를 새어 나가게 두지 않는다 | 응답을 DTO 로 캐스팅하다 나는 `TypeError` 처럼 HTTP 계층을 거치지 않는 예외도 API 호출 래퍼에서 공통 예외로 좁힌다. 그러지 않으면 `on ApiException` 만 잡는 화면이 예외를 놓친다. 화면 쪽에도 마지막 `catch` 를 두고 웹과 같은 대체 문구를 띄운다 — 실패를 조용히 넘기지 않는다(R9) |
...
| **9** | **실시간 (STOMP + TickerStore)** ★성능 게이트 — 백오프·구독 복원·`_intentionalClose`·lifecycle(`paused` 한정)·connectivity 디바운스, 16ms flush, 플래시 스윕, 1초 스로틀 재정렬 + 스크롤 중 동결, 탭 비활성 시 unsubscribe. **+ 원시 틱 관찰자 훅**(`addRawObserver`/`removeRawObserver` — §5.4 ②): 티커 소비자는 목록 하나가 아니라 **목록과 캔들 차트 둘**이며, 차트는 프레임 버퍼가 아니라 **매 틱**을 받아야 한다. 훅과 `_chartRevision` 알림 지점까지 이 단위에서 만든다(관찰자는 10단위에서 붙인다) | **아래 §7 측정 기준 3개를 전부 통과한다. 통과하지 못하면 10단계로 넘어가지 않는다.** 성능 계약 테스트 + 행 격리 위젯 테스트 통과. 관찰자를 등록하지 않은 상태에서 목록 성능이 9단위 이전과 동일하다(관찰자 목록이 빈 경로) |
| **10** | **코인 상세 + 실시간 캔들 차트** — `CandleScale`·`normalizeCandleTime`·`LiveCandleFolder.fold`·`MergedCandles` **전부 순수 Dart** + 단위 테스트, REST 캔들 `FutureProvider.family`(키 = `CandleRequest`), `TickerStore.addRawObserver`/`removeRawObserver` 등록·해제와 `hold()`/`release()` 점유, `_chartRevision` 구독(프레임당 1회 재계산), 15초 재조정 타이머, `CustomPainter` 2레이어, 간격 칩 6종, 패닝·핀치줌·롱프레스 크로스헤어, **커서 기반 과거 구간 페이징**(§4.3.11), 가로 풀스크린, 헤더/차트 `RepaintBoundary` | ① 봉 접기 단위 테스트: 한 프레임에 `[100, 130, 90, 110]` 이 들어오면 그 봉이 `high=130, low=90, close=110` 이다(**프레임 버퍼로 접으면 `high=low=close=110` 이 되어 실패한다**). ② `mergeLiveCandles` 3분기(과거 무시 / 같은 봉 = 서버 open + 실시간 high·low·close / 미래 봉 append)와 서버 캔들 0개 경우가 테스트로 고정된다. ③ Timeline: 초당 수백 틱에서 **캔들 레이어 repaint 가 프레임당 1회를 넘지 않는다.** ④ 크로스헤어 드래그 중 캔들 레이어가 재페인트되지 않는다. ⑤ 과거로 패닝해 실시간 봉이 화면 밖이면 **repaint 가 0이다.** ⑥ 새 봉이 열리고 15초 뒤 REST 재조회가 정확히 **1회** 나가고, 보고 있던 구간이 그대로다 |
...
| # | 대상 | 도구 | 왜 여기인가 |
|---|---|---|---|
| **1** | **포매터 전량 · 한글 · 시각 · Decimal** | 순수 단위 | §8.5 의 입출력 표가 그대로 케이스다. `formatKRW(15000) → "2만원"`, `formatKRW(199999900) → "2억원"`(`1억 10,000만원` 이 나오면 안 된다), `formatCurrency(123456789, "KRW") → "123,456,789원"`(축약하지 않는다), `formatCurrencyShort(123456789, "KRW") → "1억2,346만"`(공백 없음), `formatChangeRate(0) → "0.00%"`(+ 없음), `formatQuantity(-5) → "-5.0000"` 같은 반직관 케이스는 테스트 없이는 **반드시** 어긋난다. 한글은 초성=`startsWith`/자모=`contains` 비대칭. Decimal 은 `1e-8` 지수 표기 경계 |
...
| **6** | **`CandleScale` · 복기 시뮬레이션 · 라운드 규칙** | 순수 단위 | `getX`/`getY`/히트테스트/줌 앵커 보존. 복기 시뮬레이션은 위반 손실 누적 + 실현 배수 + `round()` — "웹과 값이 일치해야 한다"가 명시적 요구다(§6.3.4). 긴급 상한 100만·규칙 중복 차단(R8) |
