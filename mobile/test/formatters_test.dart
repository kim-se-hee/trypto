...
void main() {
  ...
  group('formatCurrency (§8.5.3)', () {
    const cases = <(double, String, String)>[
      ...
      (123456789, 'KRW', '123,456,789원'), // 축약하지 않고 원 단위 전액
  ...
  group('formatCurrencyCompact (§8.5.4)', () {
    const cases = <(double, String, String)>[
      (1234.5, 'USDT', r'$1,234.50'),
      (123456789, 'KRW', '123,456,789'), // 축약하지 않고 원 단위 전액
      (1234, 'KRW', '1,234'),
    ];
    for (final (value, currency, expected) in cases) {
      test(
  ...
  group('formatCurrencyShort', () {
    const cases = <(double, String, String)>[
      (99999999, 'KRW', '1억'),
      (123456789, 'KRW', '1억2,346만'), // 억과 만 사이에 공백이 없다
      (8500.5, 'KRW', '8,501'), // 1만 미만은 반올림한 정수만
      (-1234567, 'KRW', '-123만'),
      (1234567, 'USDT', r'$1.2M'),
      (12345, 'USDT', r'$12.3K'),
      (-1234567, 'USDT', r'-$1.2M'), // 부호가 달러 기호 앞에 온다
      (12.345, 'USDT', r'$12.35'),
    ];
    for (final (value, currency, expected) in cases) {
      test(
        '($value, $currency) → $expected',
        () => expect(formatCurrencyShort(value, currency), expected),
      );
    }
  });
  ...
  group('formatPrice (§8.5.7)', () {
    const cases = <(double, String, String)>[
      ...
      (1.23456, 'USDT', '1.2346'),
      (0.5, 'USDT', '0.5000'),
      (0.05432, 'USDT', '0.05432'),
      (0.000012345678, 'USDT', '0.00001235'),
      (2.5, 'SOL', '2.5000'),
      (0.5, 'SOL', '0.5000'),
      (0.00001, 'SOL', '1.00e-5'),
    ];
