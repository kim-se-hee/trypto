-- 부하테스트 시드 템플릿. reset.sh 가 1000 를 치환해 base.sql 로 렌더한다.
-- Hibernate DDL(create) 이후 spring.sql.init 으로 적재됨 (SQL_INIT_DATA_LOCATIONS=file:/seed/base.sql).
-- 앱 클래스패스 밖(loadtest/seed)에 있어 운영 이미지에는 실리지 않는다.
-- k6 시나리오: walletId 1~1000, exchangeCoinId 1~10
-- DB 모델: wallet UK(round_id, exchange_id), round 는 user 당 round_number 유니크
--         → 1000 wallet 을 위해 user/round 도 같은 수로 1:1 매핑
