...
regretanalysis 응용 서비스는 타 컨텍스트 UseCase 를 직접 주입하지 않는다. investmentround·wallet·trading·marketdata·portfolio 의 UseCase 는 regretanalysis 자기 컨텍스트의 ACL 출력 포트(`InvestmentRoundQueryPort`·`WalletQueryPort`·`TradingQueryPort`·`MarketDataQueryPort`·`PortfolioQueryPort`)와 그 구현 `RegretAnalysisAcl...QueryAdapter` 가 감싸 regretanalysis 자기 모델/VO 로 번역해 노출한다.
...
### MarketDataQueryPort ← MarketData (`RegretAnalysisAclMarketDataQueryAdapter`)
