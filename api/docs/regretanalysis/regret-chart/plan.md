...
- 서버는 전체 원칙을 준수했을 때의 시뮬레이션 자산(`ruleFollowedAsset`)만 내려준다. 원칙 조합별 자산은 미리 계산하지 않는다. 가능한 조합의 수가 많고, 계산에 필요한 재료(원칙별 위반 손실과 발생일·실현일, 긴급 충전 내역)는 두 응답에 이미 모두 들어 있기 때문이다
- 원칙을 골라 켠 곡선은 클라이언트가 만든다. 복기 리포트의 위반 거래 목록에서 켜 둔 원칙 몫만 골라, 미실현분은 금액으로 더하고 실현분은 실현일의 배수로 곱한다. 배수는 이 응답의 `emergencyCharges` 와 함께 날짜순으로 다시 굴린다. 서버가 `ruleFollowedAsset` 을 만드는 방식과 같은 계산이다
...
원칙 골라 보기가 배수를 다시 굴릴 때 쓴다. 긴급 충전은 위반과 무관한 새 돈이라 충전일의 배수를 1 쪽으로 되돌린다.

| 필드 | 타입 | 설명 |
|------|------|------|
| chargedDate | LocalDate | 충전 날짜 |
| amount | BigDecimal | 충전 금액 (원화) |
