...
- [ ] 라운드에 긴급 자금 필드(`emergency_funding_limit`, `emergency_charge_count`) 추가 및 라운드 시작 시 횟수 3 초기화
- [ ] `ChargeEmergencyFundingUseCase`와 서비스 구현(멱등성 검사·소유자 검증·도메인 4단계 검증)
- [ ] 라운드 조회/저장(횟수 감소, 낙관적 잠금) 연동
- [ ] 대상 지갑 식별(`roundId + exchangeId`) 및 기축통화 코인 조회 연동
- [ ] 잔고 증가 + 긴급 자금 이력 저장을 단일 트랜잭션으로 반영
- [ ] 긴급 자금 투입 REST 어댑터와 요청/응답 DTO
- [ ] `ErrorCode`와 `messages.properties`에 신규 에러 코드 반영
