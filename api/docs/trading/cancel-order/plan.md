...
주문 CAS UPDATE와 잔고 unlock은 반드시 같은 트랜잭션에서 수행한다. 주문은 CANCELED인데 잔고가 lock된 상태를 방지한다.

```
@Transactional 범위:
    ├─ 주문 조회 + 소유권 검증
    ├─ CAS UPDATE (PENDING → CANCELED)
...
sequenceDiagram
    ...
    OrderPort->>MySQL: UPDATE orders SET status='CANCELED' WHERE status='PENDING'
    ...
    Controller-->>Client: 200 OK {orderId, status: CANCELED}
