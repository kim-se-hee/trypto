...
- [ ] Candle / CandleInterval 도메인 모델 정의 (interval → measurement 매핑, 문자열 → enum 변환)
- [ ] CandleQueryPort 출력 포트 정의 (커서 기반 조회, 시간 오름차순 정렬)
- [ ] CandleQueryAdapter 구현 (InfluxDB 클라이언트로 직접 쿼리)
- [ ] FindCandlesUseCase / FindCandlesService 구현 (조회 오케스트레이션)
- [ ] CandleController 와 요청/응답 DTO (interval·limit 검증, FindCandlesQuery 변환, CandleResponse 변환)
