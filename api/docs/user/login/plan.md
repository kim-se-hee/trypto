...
- 소셜 신원 값은 `social_identity` 테이블에 존재하고, `user` 는 이를 ID 로 참조한다.
...
| 컬럼 | 제약 | 의미 |
|---|---|---|
| social_identity_id | NOT NULL  | 이 회원을 만든 소셜 신원. 불변 기록으로 탈퇴 후에도 남아 재가입 제한 판정에 쓰인다 |

- 로그인의 기존 회원 찾기는 `social_identity` 를 조회한다. `user_id` 가 채워져 있으면 그 회원으로 로그인한다. 탈퇴한 회원은 연결이 해제되어 있으므로 로그인되지 않는다.
...
- `social_identity` 의 (provider, provider_id) 유니크 제약이 같은 소셜 계정의 행을 하나로 강제한다.
