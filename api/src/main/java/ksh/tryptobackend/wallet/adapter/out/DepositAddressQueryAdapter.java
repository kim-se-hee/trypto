...
public class DepositAddressQueryAdapter implements DepositAddressQueryPort {
    ...
    public Optional<DepositAddress> findByRoundIdAndAddress(Long roundId, String address) {
        ...
        DepositAddressJpaEntity entity =
                queryFactory
                        .selectFrom(da)
                        .join(w)
                        .on(da.walletId.eq(w.id))
                        .where(w.roundId.eq(roundId), da.address.eq(address))
                        .fetchOne();

        return Optional.ofNullable(entity).map(DepositAddressJpaEntity::toDomain);
    }
}
