...
-- user 1~1000
INSERT INTO user (user_id, nickname, created_at, updated_at)
WITH RECURSIVE seq AS (
  SELECT 1 AS n UNION ALL SELECT n + 1 FROM seq WHERE n < 1000
)
SELECT n, CONCAT('loadtest', n), NOW(), NOW() FROM seq;
...
SELECT 1000 + n, CONCAT('loadtest', 1000 + n), NOW(), NOW() FROM seq;
