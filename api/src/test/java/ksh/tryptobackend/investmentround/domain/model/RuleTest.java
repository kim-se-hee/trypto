...
class RuleTest {
    ...
    void create_rateRuleWithZero_throws() {
        assertThatThrownBy(() -> Rule.create(RuleType.LOSS_CUT, BigDecimal.ZERO, LocalDateTime.now()))
    ...
    void create_countRuleWithDecimal_throws() {
        assertThatThrownBy(() -> Rule.create(RuleType.AVERAGING_DOWN_LIMIT, new BigDecimal("1.5"), LocalDateTime.now()))
                .isInstanceOf(CustomException.class)
                .extracting(ex -> ((CustomException) ex).getErrorCode())
                .isEqualTo(ErrorCode.INVALID_RULE_THRESHOLD);
    }
