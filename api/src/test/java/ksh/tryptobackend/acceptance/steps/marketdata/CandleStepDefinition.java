...
public class CandleStepDefinition {
    ...
    public void 캔들_데이터가_반환된다() {
        apiClient
                ...
                .isEqualTo(2)
                .jsonPath("$.data[0].time")
                .isEqualTo("2026-03-10T00:00:00Z")
                .jsonPath("$.data[0].open")
                .isEqualTo(68500000.0)
                .jsonPath("$.data[1].time")
                .isEqualTo("2026-03-11T00:00:00Z");
    }

    @Then("캔들 데이터가 비어있다")
    public void 캔들_데이터가_비어있다() {
        apiClient.getLastResponse().expectBody().jsonPath("$.data.length()").isEqualTo(0);
    }
