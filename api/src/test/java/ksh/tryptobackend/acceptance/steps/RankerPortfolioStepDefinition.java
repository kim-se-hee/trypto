...
public class RankerPortfolioStepDefinition {
    ...
    public void 포트폴리오_순위는_이다(int rank) {
        apiClient.getLastResponse().expectBody().jsonPath("$.data.rank").isEqualTo(rank);
    }
    ...
    private void insertUsers() {
        ...
        jdbcTemplate.update(
                "INSERT IGNORE INTO user (user_id, email, nickname, portfolio_public, created_at,"
                    + " updated_at) VALUES (1, 'trader1@test.com', '트레이더1', true, ?, ?), (2,"
                    + " 'trader2@test.com', '트레이더2', false, ?, ?), (3, 'trader3@test.com', '트레이더3',"
                    + " true, ?, ?)",
                now,
                now,
                now,
    ...
    private void insertRankings() {
        ...
        jdbcTemplate.update(
                "INSERT INTO ranking (user_id, round_id, period, `rank`, profit_rate, trade_count,"
                        + " reference_date, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                1L,
                1L,
                "DAILY",
                1,
    ...
    private void insertPortfolioSnapshot() {
        ...
        jdbcTemplate.update(
                ...
                new BigDecimal("90000000.00000000"),
                new BigDecimal("95000000.00000000"),
                new BigDecimal("5.5600"),
        ...
        jdbcTemplate.update(
                ...
                new BigDecimal("4000000.00000000"),
                new BigDecimal("4600000.00000000"),
