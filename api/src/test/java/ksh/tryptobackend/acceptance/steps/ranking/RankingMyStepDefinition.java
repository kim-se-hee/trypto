...
public class RankingMyStepDefinition {
    ...
    public void 내_랭킹_테스트_데이터가_준비되어_있다() {
        jdbcTemplate.execute("INSERT IGNORE INTO user (user_id, social_identity_id, nickname, portfolio_public,"
                + " created_at, updated_at) VALUES (1, 1, '테스터', true, NOW(), NOW())");
    ...
    public void 응답_data는_null이다() {
        apiClient.getLastResponse().expectBody().jsonPath("$.data").isEqualTo(null);
    }
}
