...
arguments: [subcommand, scope, feature]
---
...
frontmatter `arguments: [subcommand, scope, feature]` 로 명명된 위치 인자를 받는다.

- `$subcommand` = `draft` 또는 `refine`
- `$scope` = 기능이 속한 위치 (spec 스킬과 동일)
...
`docs/<scope>/<feature>/spec.md` 가 존재해야 한다. 없으면 사용자에게 알리고 종료한다 — 계획은 스펙을 구현 대상으로 삼는다.
