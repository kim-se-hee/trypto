...
threshold **합격**이면 루프 도중 추가한 계측 지표를 전부 떼어낸다. 성능 검증과 병목 특정을 위해 추가한 지표는 역할이 끝났으므로, 쓸모가 있었더라도 코드에 남기지 않는다. 해당 지표를 추가한 커밋 위에 다른 커밋이 쌓였을 수 있어 `reset` 으로 되감으면 멀쩡한 커밋까지 날아가므로, `git revert --no-edit <계측 커밋>` 으로 그 커밋만 취소한다. 지표가 무엇을 짚어냈는지는 `$REPORT` 의 "추가했다 제거한 지표" 에 적어 근거를 남긴다. 상시 모니터링으로 승격할 가치가 있어 보이는 지표가 있으면 코드에 남기는 대신 `$REPORT` 에 제안으로만 적는다 — 남길지는 사용자가 정한다.
...
내용 없는 섹션은 통째로 뺀다. 추가했다 제거한 지표가 없으면 그 섹션을 적지 않고, 되돌린 시도와 아키텍처 제안도 마찬가지다.
