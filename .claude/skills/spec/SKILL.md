...
spec.md 전체가 곧 비지니스 규칙이므로 `## 비지니스 규칙` 우산 헤딩은 두지 않는다. 규칙은 유형별 `##` 섹션으로 직접 둔다.

spec 은 비즈니스 언어로만 쓴다. 에러 코드 상수, DB 테이블/컬럼명, UseCase·클래스명 같은 구현 토큰은 spec 에 쓰지 않고(그건 plan 몫) 규칙을 도메인 용어로 표현한다. 검증 규칙은 spec, 그 규칙에 매핑되는 에러 코드는 plan 의 API 명세에 둔다.

```markdown
## 목적
<이 기능이 무엇이고 왜 필요한지. 조회 기능이면 "무엇을 반환하는지"는 목적에 적지 말고 아래 조회 항목으로 뺀다.>

## 조회 항목        ← 조회(read) 기능만. 명령 기능이면 생략한다
- <반환·제공하는 데이터 항목>

## <규칙 그룹>        ← 검증/계산/상태 전이/권한 등 유형별로 바로 ## 에 둔다
- <규칙 본문>
- <규칙 본문>
  ...
```
