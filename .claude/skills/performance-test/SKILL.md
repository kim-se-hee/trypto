...
- `$scenario` = `loadtest/k6/scenarios/` 안의 파일명 (예: `ticker_subscription.js`, `place_order.js`) — 필수
- `$loadgen_count` = loadgen 인스턴스 대수 — 선택, 기본 1

호출 예:

```
/performance-test ticker_subscription.js       # loadgen 1대 (5000 VU 한 인스턴스에 몰림)
/performance-test ticker_subscription.js 4     # loadgen 4대 (1대당 1250 VU)
```
...
# k6 이미지의 빌드 입력 = Dockerfile 하나뿐. xk6-sse 는 latest 로 가져오므로
# Dockerfile 텍스트가 같으면 같은 태그 → Hub 에 있는 이미지 재사용.
hash_file() {
...
K6_H=$(hash_file loadtest/k6/Dockerfile)
...
# 항상 .env 는 새 태그로 덮어쓴다 (Hub 존재 여부와 무관하게 SUT/loadgen 이 pull 할 태그를 명시)
{
  ...
  echo "K6_IMAGE=kimsehee98/trypto-k6:$K6_TAG"
} > .env

# 이 스킬 step 안에서도 K6_IMAGE 를 export 해 두면 step 11 에서 그대로 쓴다.
export K6_IMAGE="kimsehee98/trypto-k6:$K6_TAG"

# Hub 에 이미 있는지 확인. 모두 있으면 빌드/푸시 통째 스킵.
need_build_api=1; need_build_col=1; need_build_eng=1; need_build_k6=1
...
docker manifest inspect "kimsehee98/trypto-k6:$K6_TAG"         >/dev/null 2>&1 && need_build_k6=0

if [ $need_build_api -eq 0 ] && [ $need_build_col -eq 0 ] && [ $need_build_eng -eq 0 ] && [ $need_build_k6 -eq 0 ]; then
  ...
  # k6 이미지는 compose 에 없으므로 별도 빌드 (Dockerfile 변경이 있을 때만).
  if [ $need_build_k6 -eq 1 ]; then
    docker build -t "kimsehee98/trypto-k6:$K6_TAG" -f loadtest/k6/Dockerfile loadtest/k6
  fi
  ...
  [ $need_build_k6  -eq 1 ] && (docker push "kimsehee98/trypto-k6:$K6_TAG"        ) & pids+=($!)
...
k6 Dockerfile 을 처음 빌드할 땐 xk6 가 go modules 받고 컴파일하므로 1~2분 추가로 든다 (그 다음 회차부터는 Hub 재사용으로 0).
...
`reset.sh` 가 시나리오 인자를 받아서 자동으로 적절한 compose 파일들을 같이 적용한다 (예: `ticker_subscription.js` 면 `-f docker-compose.loadtest.yml` 자동 추가하여 collector 가 처음부터 loadtest 프로파일로 뜸). 그 다음 healthy 까지 기다린다.
...
| 시나리오 | 추가 작업 |
|---|---|
| `ticker_subscription.js` | 따로 없음. ramp profile POST 는 k6 시나리오의 `setup()` 이 첫 loadgen 인스턴스에서 호출한다 (`RUN_RAMP_SETUP=true`). |
...
| 시나리오 | env |
|---|---|
| `ticker_subscription.js` | `API_HOST=$SUT_PRIVATE_IP:8080`, `COLLECTOR_HOST=$SUT_PRIVATE_IP:8081` |
...
# k6 커스텀 메트릭(sse_disconnects 등) 을 SUT 의 prometheus 로 push.
...
for i in "${!LOADGEN_IPS[@]}"; do
  ...
  # K6_IMAGE 는 step 2 에서 export 한 콘텐츠 해시 태그.
  # 커스텀 k6 (xk6-sse 포함). 모든 시나리오에서 동일 이미지 사용 — 표준 k6 모듈은 그대로 살아있다.
  ssh $SSH_OPTS ec2-user@$IP "
    docker run -d --name $CONTAINER_NAME \
      ...
      $SCENARIO_ENV \
      $PROMETHEUS_PUSH_ENV \
      $K6_IMAGE run -o experimental-prometheus-rw /scripts/scenarios/$scenario
  " &
done
wait
```
