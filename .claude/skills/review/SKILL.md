...
description: >
  ...
  /review 단독 또는 /review <prefix> <feature> 로 호출한다.
...
frontmatter `arguments: [prefix?, feature?]` 둘 다 옵셔널.

- 인자 없음 → 현재 브랜치 변경분만 본다. 체크리스트는 `.review/issues.md` 임시 위치에 둔다.
- 둘 다 지정 → ddd-reviewer 에 `docs/<prefix>/<feature>/spec.md` 와 `plan.md` 를 같이 넘겨 BR 매핑까지 검증. 체크리스트는 `docs/<prefix>/<feature>/review.md` 에 저장한다 (PR 추적용).
...
prefix 만 주거나 feature 만 주는 부분 입력은 거부하고 사용자에게 호출 형태를 안내 후 종료한다.

## 사전 제약

`git diff main...HEAD` 가 비어 있고 미커밋 변경도 없으면 "리뷰할 변경 없음" 알리고 종료한다.

`prefix`/`feature` 가 주어졌는데 `docs/<prefix>/<feature>/spec.md` 가 없으면 사용자에게 알리고 종료한다.

기능 디렉토리(`docs/<prefix>/<feature>/`)에 `review.md` 를 먼저 만든다.  이 파일에 리뷰 사항을 기록한다. (인자 없는 호출은 `.review/issues.md`)
...
각 호출 프롬프트에 `prefix`, `feature` (있으면) 만 전달한다. 각 리뷰어는 자기 출력 형식대로 마크다운 본문을 메인에 직접 반환한다.
...
1. `review-applier` 호출. 프롬프트에 `prefix`, `feature`, `이슈 본문` (체크박스 한 항목 전체) 을 전달.
...
다음 조건이 모두 참일 때만 `docs/<prefix>/<feature>/index.md` 의 `단계` 줄을 `단계: review` 로 갱신한다.

- `prefix`/`feature` 인자가 주어졌다 (인자 없는 `/review` 는 갱신 대상이 없음).
