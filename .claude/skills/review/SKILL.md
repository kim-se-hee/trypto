...
- 둘 다 지정 → ddd-reviewer 에 기능 디렉터리의 `spec.md` 와 `plan.md` 를 같이 넘겨 BR 매핑까지 검증. 체크리스트는 기능 디렉터리의 `review.md` 에 저장한다 (PR 추적용).
...
`scope`/`feature` 가 주어졌는데 기능 디렉터리의 `spec.md` 가 없으면 사용자에게 알리고 종료한다.

기능 디렉터리에 `review.md` 를 먼저 만든다. 이 파일에 리뷰 사항을 기록한다. (인자 없는 호출은 `.review/issues.md`)
...
다음 조건이 모두 참일 때만 기능 디렉터리의 `index.md` 의 `단계` 줄을 `단계: review` 로 갱신한다.
